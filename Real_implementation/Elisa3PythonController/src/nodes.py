...
class Camera:
    def __init__(self, tag):
        ...
        self.tag = tag
        self.timer = 0.0
        self.listener_camera = rospy.Subscriber('Bebop{}/ground_pose'.format(self.tag + 1), Pose2D,
                                                self.listen_optitrack_callback)
        self.listener_camera_timer = rospy.Subscriber('Bebop{}/pose'.format(self.tag + 1), Pose,
                                                      self.listen_optitrack_timer_callback)
...
class Node:
    def __init__(self, release_time, range=range, tag=0):
        ...
        self.orien = np.array(
            [self.start_orien, self.start_orien, self.start_orien, self.start_orien, self.start_orien])
        ...
        self.listener_robot_speed = rospy.Subscriber('swarm/elisa3_robot_{}/speed'.format(self.tag), Float64,
                                                    self.listen_robot_speed_callback)

        self.listener_robot_heading = rospy.Subscriber('swarm/elisa3_robot_{}/heading'.format(self.tag), Float64,
                                                    self.listen_robot_heading_callback)
    ...
    def listen_robot_speed_callback(self, speedMsg):
        self.speed = float(speedMsg.data)
        msg = """
                ID: {} 
                speed: {}
                  """.format(self.tag,
                             self.speed)
        #print("msg control: ", msg)

    def listen_robot_heading_callback(self, headingMsg):
        self.angle = float(headingMsg.data)
        msg = """
                ID: {} 
                angle: {}
                  """.format(self.tag,
    ...
    def go_to_goal(self):
        ...
        omega = self.setup["K_p"] * math.atan2(math.sin(phi_d - self.estimation[2]), math.cos(
            phi_d - self.estimation[2]))  # Only P part of a PID controller to give omega as per desired heading
    ...
    def measurement_fusion(self, odom_measurement, cam_measurement):
        ...
        if (self.t % 5 == 0):
            if (sr_KALMAN and ~mr_KALMAN):
                optimal_state_estimate_k, covariance_estimate_k = self.kalman_cam.sr_EKF(cam_measurement,
                                                                                         self.estimation, 1)
            elif (mr_KALMAN and ~sr_KALMAN):
                optimal_state_estimate_k, covariance_estimate_k = self.kalman_cam.mr_EKF(cam_measurement,
                                                                                         self.estimation, 1)
            self.measurement_Kalman = optimal_state_estimate_k
    ...
    def loop_fuc(self, cameras, move_type):
        ...
        self.estimation = self.states_transform(self.estimation, self.speed, self.angle)
        ...
        #self.print_position_measures()
        self.msg_auto_motive = np.array(self.estimation)

        #self.theoretical_position = self.states_transform(self.theoretical_position, self.speed, self.angle)
        # self.estimation = self.states_transform(self.estimation, self.speed, self.angle)
...
class Nodes:
    def __init__(self, active_robots=['0000']):
        ...
        self.k = 1
        self.xpos_range = np.flip(np.linspace(0.8953, 1.326, num=20))
        self.ypos_range = np.linspace(1, 1.121, num=20)
