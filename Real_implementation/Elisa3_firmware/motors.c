...
*/
	...
	if(computeOdometry>=2) {	// compute odometry when we get the last encoders values for both wheels
		...
		if(robotPosition == HORIZONTAL_POS) {
			...
			//theta += ((rightDist-rightDistPrev)-(leftDist-leftDistPrev))/WHEEL_DIST;
			theta = theta + (rightDist-leftDist)/WHEEL_DIST;
			//filtered_theta = filtered_theta + (rightDist-leftDist)/WHEEL_DIST;
			rightDist = 0;
			leftDist = 0;
			if (theta > 3.14159){
				theta = theta - 6.28318;
			...
			//if (filtered_theta > 3.14159){
			//	filtered_theta = filtered_theta - 6.28318;
			//}else if (filtered_theta < -3.14159){
			//	filtered_theta = filtered_theta + 6.28318;
			//}			
		} else {
			//thetaOld = thetaAcc;
		...
		position[0] = position[0] + cos(theta)*deltaDist;
		position[1] = position[1] + sin(theta)*deltaDist;
