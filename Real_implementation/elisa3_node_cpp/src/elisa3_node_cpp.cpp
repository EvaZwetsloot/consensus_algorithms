...
std::ofstream fw("/home/steven/catkin_ws/src/elisa3_node_cpp/src/data_run11.txt", std::ofstream::out);
...
void Robot::updateSensorData() {
    ...
    robSpeed = getSpeed(address)*5;
     ...
     //  std::cout << "xPos " << tag << " :: [" << getOdomXpos_temp1(address) << ", " << getOdomYpos_temp1(address) << "], Events::" << getNbEvents(address) << ", " <<getNbResets(address) <<std::endl;
     //  std::cout << "xPos " << tag << " :: [" << robXPos_fixed << ", " << robYPos_fixed << "], temp::" << pos_temp << ", " << comm_diff_temp <<std::endl;
     ...
     // std::cout << "" << std::endl;//
        ...
        if(tag == 2){
            if (getOdomXpos_temp1(address) == robXPos_fixed){
       ...
       //useEstimate = 0;
        ...
        if (((time_now.tv_sec*1000) + (time_now.tv_usec/1000))-((last_time.tv_sec*1000) + (last_time.tv_usec/1000)) > 10){
            fw << tag << ", " << ((time_now.tv_sec*1000) + (time_now.tv_usec/1000)) << ", " << robTheta << ", "<< getOdomTheta_filtered(address) << ", " << xPos_filtered*1000 << ", " << robXPos << ", " << yPos_filtered*1000 << ", " << robYPos << ", " << robTurning << ", "<< theta_filtered << ", "<< theta_estimate <<  ", "<< speed_max << ", " << getNbEvents(address) << ", " << getNbResets(address) << ", "<< comm_diff << ", "<< useEstimate << "," << robXPosAfterTurn << ", " << robYPosAfterTurn << ", "<< comm_diff_temp << ", " << reset_count2 <<"\n";
            if (tag == robots-1){
...
void handlerAllReset(const std_msgs::Float64MultiArray::ConstPtr& msg) {
    ...
    for (it = robots_dict.begin(); it != robots_dict.end(); it++){
       if(robots_dict[it->first].useEstimate == 1){
          // Only reset when the robot is moving straight
